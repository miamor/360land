...
function runChat() {
  ...
  var getDone = function(chatsource) {
    if (typeof chatsource !== "undefined") {
      if (chatsource.indexOf("<!DOCTYPE html PUBLIC") === 0) {
        ...
        $("a.return", "#chatbox-members").each(function() {
          // Duyệt từng thành viên trong danh sách
          ...
          if (user_id === my_user_id) {
            // Nếu user_id trùng với my_user_id
            ...
            if (my_chat_level == 2) {
              // Mình có quyền quản lý
              if (user_chat_level != 2) {
                // Nick này cấp bậc thấp hơn mình
              ...
              if (
                ...
                // Nick này có quyền quản lý nhưng cấp thấp hơn mình
                quickAction($list, "unmod", user_name, "Xóa quyền quản lý");
              } else if (my_user_level == 1 && user_chat_level != 2) {
                // Nick này chưa có quền quản lý và cấp thấp hơn mình
  ...
  $("#chatbox-members, #chatbox-title").on(
    "click",
    ".chatbox-action",
    function() {
  ...
  if (teamID) {
    $(
      "#chatbox-tabs, #chatbox-me, #chatbox-option-color, #chatbox-option-smiley, #chatbox-header, #chatbox-option-submit"
    ).remove();
